...
class MedicalImageDataset(Dataset):
    def __init__(self, data_list_path, sigma=2.0, transform=None, device='cpu', cache_dir='data_cache'):
        ...
        dataset_name = Path(data_list_path).stem  # e.g. train, val, test
        ...
        if self.cache_file.exists():
            ...
            print(f"Loaded {len(self.data_cache)} samples from cache")
        else:
            # Load from original files and create cache
            print(f"Cache not found, loading from original files...")
            self.data_cache = self._load_from_original()
            print(f"Saving cache to {self.cache_file}...")
            self._save_cache()
            print(f"Cache saved, next startup will be faster")
    ...
    def _create_heatmap(self, label_path):
        """Create Gaussian heatmap from label data"""
        label_data = pd.read_csv(label_path, sep=r'\s+', header=None, 
                               names=['x', 'y', 'value'])
        
        heatmap = np.zeros((64, 64))
        
        # Find all positive points (value > 0)
        positive_points = label_data[label_data['value'] > 0]
        
        if len(positive_points) > 0:
            # Create meshgrid for Gaussian calculation
            xx, yy = np.meshgrid(np.arange(64), np.arange(64))
            
            for _, point in positive_points.iterrows():
                center_x, center_y = int(point['x']), int(point['y'])
                
                # Create Gaussian kernel centered at (center_x, center_y)
                gaussian = np.exp(-((xx - center_x)**2 + (yy - center_y)**2) / (2 * self.sigma**2))
    ...
    def _load_from_original(self):
        ...
        for idx in range(len(self.data_list)):
            ...
            # Load original label points (don't generate Gaussian heatmap)
        ...
        print(f"Loaded {len(data_cache)} samples from original files")
        return data_cache
    
    def _save_cache(self):
        """Save data to cache file"""
        try:
            # Move data to CPU before saving to avoid cross-device issues
            cpu_data = []
    ...
    def __getitem__(self, idx):
        ...
        target = self._apply_gaussian_smoothing(sparse_label)
        
        # If data augmentation is needed, apply transform here
        # Note: data augmentation needs to be done on device because data is already on device
        if self.transform:
            inputs = self.transform(inputs.clone())  # clone to avoid modifying cached data
        
        return inputs, target
    
    def _apply_gaussian_smoothing(self, sparse_label):
        ...
        heatmap_tensor = torch.from_numpy(heatmap).float().unsqueeze(0).to(self.device)
