...
class GeneralizedCrossEntropyLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        # For binary classification, pt = p when target=1, pt = 1-p when target=0
        pt = torch.where(target == 1, input, 1 - input)

        # GCE loss function: (1 - pt^q) / q
        # Use limit form when q approaches 0
        if self.q == 1.0:
            # When q=1, GCE degenerates to standard BCE
            loss = -torch.log(pt)
        else:
...
def get_loss_function(loss_type='gce', **kwargs):
    ...
    Args:
        loss_type (str): Loss function type, options: 'gce' or 'bce'
...
class FocalLoss(nn.Module):
    ...
    Loss function that focuses on hard samples by adjusting gamma and alpha parameters to balance positive/negative samples and easy/hard samples.
